...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata name) public payable {
        ...
        string memory _finalSvg = string(abi.encodePacked(svgPartOne, _name, svgPartTwo));
        uint256 newRecordId = _tokenIds.current();
        uint256 length = StringUtils.strlen(name);
        string memory strLen = Strings.toString(length);
        console.log("Registering %s on the contract with id %d", _name, newRecordId);

        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "', _name,
                '", "description": "A domain on the ', tld, ' name service"',
                '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(_finalSvg)),
                '","length":"', strLen, '"}'
            )
        );
